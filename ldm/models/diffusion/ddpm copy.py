...
class LatentDiffusionSR(DDPM):
    ...
    def p_losses(self, lq, cond, gt, t, noise=None):
        noise = default(noise, lambda: torch.randn_like(cond))
        ...
        logvar_t = self.logvar[t.data.cpu()].to(self.device)
        loss = loss_simple / torch.exp(logvar_t) + logvar_t
        # loss = loss_simple / torch.exp(self.logvar) + self.logvar
        if self.learn_logvar:
            loss_dict.update({f'{prefix}/loss_gamma': loss.mean()})
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        log = dict()
        z, c, z_gt, x, xrec, y, xc = self.get_input(batch, self.first_stage_key,
                                           return_first_stage_outputs=True,
                                           force_c_encode=True,
                                           return_original_cond=True,
                                           bs=N)
